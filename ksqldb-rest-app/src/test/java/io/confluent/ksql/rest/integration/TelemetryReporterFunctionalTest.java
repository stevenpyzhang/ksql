...
public class TelemetryReporterFunctionalTest {
  ...
  private static final TestKsqlRestApp REST_APP = TestKsqlRestApp
      ...
      .withProperty("confluent.telemetry.metrics.collector.interval.ms", 6000)
  ...
  @Before
  public void setUp () {
    consumer = createNewConsumer(TEST_HARNESS.kafkaBootstrapServers());
  ...
  @Test
  public void shouldExecuteInfoRequest() throws InterruptedException {
    // When:
    ...
    while (System.currentTimeMillis() - startMs < 20000) {
      ...
      for (ConsumerRecord<byte[], byte[]> record : records) {
        ...
        System.out.println(resourceLabels);
        // Check that the labels from the config are present.
        assertThat(resource.getType(), equalTo("ksql"));

        if (m.getMetricDescriptor().getName().startsWith("io.confluent.ksql.metrics")) {
