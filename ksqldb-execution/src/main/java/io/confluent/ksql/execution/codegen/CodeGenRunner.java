  ...
  private final class Visitor extends TraversalExpressionVisitor<Void> {
    ...
    public Void visitFunctionCall(final FunctionCall node, final Void context) {
      ...
      for (final Expression argExpr : node.getArguments()) {
        if (argExpr instanceof LambdaFunctionExpression) {
        ...
        if (argExpr instanceof UnqualifiedColumnReferenceExp) {
          SqlArray inputArray = (SqlArray) newSqlType;
          inputType = inputArray.getItemType();
        } else if (newSqlType == SqlTypes.LAMBDALITERAL && holder.getMetadata().getCategory().equals("STRING")) { //probably need to extend to all function types, if there's one input and nothing else it needs to be the same as the input type
