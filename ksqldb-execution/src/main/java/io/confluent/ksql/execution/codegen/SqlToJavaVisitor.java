...
public class SqlToJavaVisitor {
  ...
  private class Formatter implements 
      ExpressionVisitor<Pair<String, SqlType>, SqlToJavaTypeContext> {
    ...
    public Pair<String, SqlType> visitLambdaExpression(
      ...
      for (final String lambdaArg: exp.getArguments()) {
        argPairs.add(new Pair<>(
            ...
            SchemaConverters
                .sqlToJavaConverter()
                .toJavaType(context.getLambdaType(lambdaArg))));
      }
      return new Pair<>(LambdaUtil.function(argPairs, lambdaBody.getLeft()),
          expressionTypeManager.getExpressionSqlType(
              exp,
              context.getLambdaTypeMapping(),
    ...
    public Pair<String, SqlType> visitLambdaVariable(
        final LambdaVariable lambdaVariable,
        final SqlToJavaTypeContext context
    ) {
      return new Pair<>(
    ...
    public Pair<String, SqlType> visitFunctionCall(
      ...
      for (final Expression argExpr : node.getArguments()) {
        final SqlType newSqlType = expressionTypeManager.getExpressionSqlType(
            argExpr,
            context.getLambdaTypeMapping(),
            context.getInputTypes());
